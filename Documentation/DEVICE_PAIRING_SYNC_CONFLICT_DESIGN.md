...
- Parent Trigger: projected子の作成は親が`projected|applied`、appliedへの遷移は全親が`applied`であり、同じOrigin Device／Vehicle、同じAlias ID／Projection Version／Canonical Vehicle、親のOrigin／Materialized Entity IDが子の対応列と一致する場合だけ許可します。conflicted、quarantined、superseded_projection親を常に拒否し、未適用親では子をappliedにできません。self／secondary／tertiary自己参照も拒否します。projected -> applied Triggerは対象業務行を同じtransaction内で読戻し、Version付きcanonical encoderで再計算したDigestが保存済み`materialized_content_digest`と一致することも必須にします。
...
- Check: 全Version／generation >=1、Digest 32 byte。appliedのKeychain参照／日時、quarantinedのquarantine IDを状態と明示的に同値にします。
