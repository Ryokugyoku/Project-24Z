...
|---|---|---|---|---|---|
| macOS | USB Serial | OS提供serial deviceを安全にopenするData Adapter。system driverで不足する場合だけDriverKit／USBSerialDriverKitを別検討 | 2026-07-19にOBDLink EX EX101を対象Macで列挙し、115200/8N1でAdapter単体双方向通信を確認。詳細は`REAL_VEHICLE_USB_HARD_GATE_RUNBOOK.md` | 対象AdapterがmacOSでserial endpointとして列挙され、baud、flow control、sandbox、切断復帰が成立 | `partially_verified`。列挙／open／Adapter identityは確認済み。detach、再接続、配布署名／sandbox、車両OBD応答は未確認 |
...
- OBDLink EX EX101／STN2232 v5.10.3のAdapter単体identityに限り、2026-07-19 transcriptの`??`、`ATZ`、`STDI`、`STI`をDevelopment専用Version 1 allowlistへ固定します。車両OBD、Capability probe、Raw monitor開始／停止のexact bytesは未確定であり、HG-04／HG-05通過前に追加しません。
...
2026-07-19時点で証拠として通過済みなのは、HG-01とHG-04のAdapter単体部分だけです。次の実車pilot実装は、信頼性の高い公開資料を根拠にmacOS TestFlightへ載せ、ユーザーが実車挙動を確認するための候補であり、実装・build・署名だけでHard Gate通過とは扱いません。

### 20.1 macOS TestFlight read-only実車pilot

対象は実測済み`OBDLink EX EX101`／`STN2232 v5.10.3`／115200 8N1だけです。次の順序をVersion 1 allowlistへ固定します。

1. Adapter入力境界clear、Adapter reset、Hardware／Firmware identity照合。
2. `ATE0`、`ATL0`、`ATS1`、`ATH0`、`ATSP0`で応答形式と自動Protocol選択を設定。
3. `0902`でVINを取得し、17文字・禁止文字・複数候補を検査する。地域別check digit適用は推測しない。
4. `0104`、`0105`、`010C`、`010D`を各一回実行し、値取得と標準式Decodeの両方に成功したPIDだけをHOMEへ渡す。
5. 成否にかかわらずUSB Transportをcloseする。

根拠はSAE J1979のService 01／09境界、OBD SolutionsのSTN11xx/21xx Programming Manualとread-only PID tutorialです。Service 04、08、ECU reset、DTC消去、coding、メーカー固有Request、任意AT／ST文字列、Raw CAN monitorはallowlistへ含めません。

VINとRaw VIN応答はKeychain root keyからHKDFで用途分離したAES-256-GCM鍵で暗号化し、照合は別用途鍵のHMAC-SHA256を使います。Platformへは末尾4文字だけを公開します。GRDBの識別Sessionを未割当で作り、Vehicle Identity transaction成功後に既存Session binding transactionで所属を確定します。

HOME値は一回のprobe Snapshotであり、継続live pollingやPID Chunk永続化済みとは表示しません。実車確認では、応答行分割、複数ECU、Protocol自動探索時間、NO DATA、イグニッション状態、macOS TestFlight sandbox、detachを記録し、このVersionの継続可否を判断します。
