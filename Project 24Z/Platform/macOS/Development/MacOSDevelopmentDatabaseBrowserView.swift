...
/// macOS専用の開発用read-only Database Browserです。
struct MacOSDevelopmentDatabaseBrowserView: View {
    /// Composition Rootから注入されたread-only Modelです。
    @EnvironmentObject private var model: DevelopmentDatabaseBrowserModel

    /// macOS向けselector、table、Keyboard操作可能なdetailを表示します。
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack {
                ...
                Button("更新") { Task { await model.perform(.refresh) } }.keyboardShortcut("r", modifiers: .command)
            }
            tableArea
        }
        .padding(20)
    ...
    /// macOSのtable領域内だけを縦横scroll可能にします。
            ...
            LazyVStack(alignment: .leading, spacing: 0) {
                HStack(spacing: 0) {
                    ...
                    ForEach(Array(model.state.columns.enumerated()), id: \.offset) { _, column in
                        Text(column.name).bold().frame(width: 200, alignment: .leading)
                    }
                }
                Divider()
                    ...
                    HStack(spacing: 0) {
                        ...
                        ForEach(Array(row.values.enumerated()), id: \.offset) { index, value in
                            Button(value.cellText) { Task { await model.perform(.openCell(rowID: row.id, columnIndex: index)) } }
                                .buttonStyle(.plain).lineLimit(1).frame(width: 200, alignment: .leading)
                                .accessibilityLabel("行 \(row.id + 1)、\(model.state.columns[index].name)、\(value.storageClassName)、\(value.cellText)")
                        }
                    }
                    Divider()
                }
                if model.state.hasNextPage { Button("次のpageを読み込む") { Task { await model.perform(.loadNextPage) } } }
            }
        }
        ...
        ScrollView([.horizontal, .vertical]) {
            if let detail = model.state.cellDetail {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Button("閉じる") { Task { await model.perform(.closeCell) } }.keyboardShortcut(.cancelAction)
                }.padding()
            }
